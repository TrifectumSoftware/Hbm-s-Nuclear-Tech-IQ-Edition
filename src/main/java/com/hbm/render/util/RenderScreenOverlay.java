...
public class RenderScreenOverlay {
	...
	public static void renderBadges(ScaledResolution res, Gui gui) {
		...
		if (ClientConfig.BADGES_HUD_IQ_EDITION.get()) {
			gui.drawTexturedModalRect(offsetX, offsetY, 24, 218, 24, 8);
			offsetX += width;
		}
		...
		if (true528) {
			gui.drawTexturedModalRect(offsetX, offsetY, 0, 218, 24, 8);
			offsetX += width;
		}
		if (trueExp) {
			gui.drawTexturedModalRect(offsetX, offsetY, 0, 226, 24, 8);
			offsetX += width;
		}
		if (trueRam) {
			gui.drawTexturedModalRect(offsetX, offsetY, 0, 234, 24, 8);
			offsetX += width;
		}
		if (true328) {
			gui.drawTexturedModalRect(offsetX, offsetY, 0, 242, 24, 8);
			offsetX += width;
		}
